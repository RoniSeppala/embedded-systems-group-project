...
void timer1_set_frequency(uint16_t frequency_hz) {
    ...
    if (frequency_hz == 0) {
        ...
        SET_BITS(CONTROL_REGISTER_B, TCCR1B_CLOCK_SELECT_OFFSET, TIMER1_CLOCK_OFF);
        OUTPUT_COMPARE_A = 0;
        return;
    }
    ...
    CLEAR_BITS(CONTROL_REGISTER_B, TCCR1B_CLOCK_SELECT_OFFSET, TCCR1B_CLOCK_SELECT_MASK);
    SET_BITS(CONTROL_REGISTER_B, TCCR1B_CLOCK_SELECT_OFFSET, prescaler & TCCR1B_CLOCK_SELECT_MASK);

    divider = PRESCALER_DIVIDERS[prescaler];
    ticks_per_s = ((uint32_t) F_CPU) / divider / 4;
